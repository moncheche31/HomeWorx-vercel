-- Labor pass: one resolution, labor_pct only.
...
BEGIN
  ...
  v_factor := v_loc.labor_factor;
  PERFORM public.nce_sync_estimate_location(_estimate_id);
...
-- Material pass: material_pct only, book-sourced lines only.
-- AI-estimated material carries its own factor at write time and is skipped here.
...
BEGIN
  SELECT * INTO v_loc FROM public.nce_location_factors(_estimate_id);
  v_mat_factor := v_loc.material_factor;
  ...
  FOR r IN
    SELECT li.id, li.quantity, li.unit_key::text AS unit_key, li.cost_basis,
     ...
     WHERE li.estimate_id = _estimate_id
       ...
       AND COALESCE(li.pricing_source, '') <> 'ai_estimated_material'
       AND COALESCE((li.pricing_provenance->'materialEstimate'->>'isEstimated')::boolean, false) = false
  LOOP
...
-- Equipment pass: equipment_pct only, idempotent.
...
BEGIN
  SELECT * INTO v_loc FROM public.nce_location_factors(_estimate_id);
  v_factor := v_loc.equipment_factor;
