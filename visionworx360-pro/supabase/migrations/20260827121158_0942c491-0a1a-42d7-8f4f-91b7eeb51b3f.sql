...
DECLARE
  ...
  v_quantity numeric; v_labor_hours numeric; v_labor_cost numeric;
  v_material_total numeric; v_other_total numeric; v_task_total numeric;
  v_blended_rate numeric; v_updated integer := 0; v_rows integer := 0;
BEGIN
  ...
  FOR v_task IN SELECT value FROM jsonb_array_elements(_tasks) LOOP
    ...
    v_labor_hours := CASE WHEN v_labor_hours > 0 THEN round(v_labor_hours * 4) / 4 ELSE 0.25 END;
    v_blended_rate := round(v_task_total / v_labor_hours, 2);
    ...
    UPDATE public.estimate_line_items li
       SET quantity = v_quantity,
           ...
           labor_hours_per_unit = round(v_labor_hours / v_quantity, 4),
           labor_hours_setup = 0, labor_hours = v_labor_hours,
           labor_rate = v_blended_rate, material_cost = 0,
           ...
           labor_hours_formula = format('%s total hours; blended rate preserves $%s selected ballpark task total', v_labor_hours, v_task_total),
           ...
           pricing_provenance = jsonb_strip_nulls(jsonb_build_object(
             ...
             'storageConvention', 'blended_rate_exact_task_total', 'appliedAt', now()))
