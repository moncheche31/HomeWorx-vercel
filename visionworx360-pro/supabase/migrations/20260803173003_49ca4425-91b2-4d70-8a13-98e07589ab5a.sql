...
BEGIN
    ...
    LEFT JOIN LATERAL (
      ...
      FROM (
        SELECT a.*,
          ...
          END AS base_score,
          CASE
            WHEN t.catalog_item_key IS NOT NULL AND a.assembly_key = t.catalog_item_key THEN 'catalog_key'
        ...
        -- A library item measured in a different unit than the line can never
        -- be applied by fuzzy similarity: it silently rescales the quantity.
        WHERE t.unit_key IS NULL
           OR a.unit_key IS NULL
           OR a.unit_key = t.unit_key
           OR a.assembly_key = t.catalog_item_key
  ...
  applied AS (
    ...
    SET
      ...
      -- Catalog hours are already billable labour hours per unit. Crew size is
      -- descriptive (it converts labour hours to calendar hours downstream) and
      -- must NEVER multiply the hours again.
