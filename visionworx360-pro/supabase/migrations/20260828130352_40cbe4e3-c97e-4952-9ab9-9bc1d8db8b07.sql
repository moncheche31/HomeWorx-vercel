...
AS $$
DECLARE
  r record; v_source text; v_ref text; v_book boolean; v_hit record; a record;
BEGIN
  ...
  LOOP
    v_source := NULL; v_ref := NULL; v_book := false; v_hit := NULL; a := NULL;

    SELECT ca.assembly_key, ca.source_version INTO a
    ...
    IF a.assembly_key IS NOT NULL THEN
      v_source := 'catalog_assemblies';
      v_ref := a.assembly_key;
      v_book := (a.source_version = 'nce-2026');
    ELSE
      ...
      IF v_hit.reference_id IS NOT NULL AND NOT v_hit.is_ambiguous THEN
        v_source := 'cost_reference_nce2026'; v_ref := v_hit.description; v_book := true;
      ELSIF v_hit.reference_id IS NOT NULL THEN
        v_source := 'ambiguous_book_match'; v_ref := v_hit.description;
