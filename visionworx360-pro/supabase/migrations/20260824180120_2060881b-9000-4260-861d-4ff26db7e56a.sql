CREATE OR REPLACE FUNCTION public.assembly_allowance_eligible(_assembly_key text)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $function$
  SELECT coalesce(bool_or(a.ballpark_allowance_eligible), false)
  FROM public.catalog_assemblies a
  WHERE _assembly_key IS NOT NULL
    AND a.assembly_key = _assembly_key
    AND a.is_active = true
$function$;

REVOKE ALL ON FUNCTION public.assembly_allowance_eligible(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.assembly_allowance_eligible(text) FROM anon;
REVOKE ALL ON FUNCTION public.assembly_allowance_eligible(text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.assembly_allowance_eligible(text) TO service_role;
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS _rqe_cand (
    id uuid, surface text, scope text, allowance boolean, qty numeric) ON COMMIT DROP;
  DELETE FROM _rqe_cand;

  INSERT INTO _rqe_cand (id, surface, scope, allowance, qty)
  SELECT li.id,
         public.geometry_surface_for_line(li.description, li.catalog_item_key, li.unit_key::text),
         public.quantity_scope_kind(li.description, li.catalog_item_key),
         public.assembly_allowance_eligible(li.catalog_item_key),
