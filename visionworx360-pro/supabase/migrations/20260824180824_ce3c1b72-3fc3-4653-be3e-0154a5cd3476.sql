...
BEGIN
  ...
  target AS (
    SELECT li.*,
           ...
           /* A match this engine already made stays trusted as long as it still
              describes the work: discarding it made tasks flip between priced
              and unpriced (showing $0) on every repricing run. */
           CASE WHEN li.catalog_mapping_source = 'contractor' THEN li.catalog_item_key
                WHEN li.catalog_item_key IS NOT NULL
                     AND public.catalog_mapping_is_semantic(li.description, li.catalog_item_key)
