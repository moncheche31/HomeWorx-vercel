...
BEGIN
  ...
  /* System pricing, not a contractor override: keep the override flag off. */
  ...
  LOOP
    ...
    /*
      Evidence-first: a generic rate may only be multiplied by a quantity that
      has a defensible basis. An unverified count becomes ONE reviewable
      allowance instead of fake precision.
    */
    v_evidenced := COALESCE(r.quantity_basis,'') IN
    ...
    UPDATE public.estimate_line_items SET
      ...
      pricing_provenance = COALESCE(pricing_provenance,'{}'::jsonb) || jsonb_build_object(
        'genericTradeFallback', v_rule || jsonb_build_object(
          ...
          'originalQuantity', r.quantity, 'originalUnit', r.unit_key::text,
