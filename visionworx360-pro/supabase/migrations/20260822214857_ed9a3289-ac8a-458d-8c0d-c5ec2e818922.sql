-- 1. Size-is-not-a-count detector (ADR-048 at the estimate-line level).
CREATE OR REPLACE FUNCTION public.quantity_is_size_not_count(
  _description text, _quantity numeric, _unit text)
RETURNS boolean
LANGUAGE sql IMMUTABLE
SET search_path = public
AS $$
  SELECT COALESCE(_unit,'each') IN ('each','lump_sum','allowance','other')
     AND COALESCE(_quantity,0) > 12
     AND _quantity = trunc(_quantity)
     AND COALESCE(_description,'') ~* (
       '(^|[^0-9.])' || trim(to_char(_quantity, 'FM999999')) ||
       '\s*(\"|”|''|’|x\s*[0-9]|-?\s*(in|inch|inches|ft|foot|feet|mm|cm|lf|sf|sq)\b)')
$$;

-- 2. The write gate learns the quantity-sanity rule.
...
DECLARE
  ...
  v_qty_owned boolean := NEW.quantity_reviewed_at IS NOT NULL
                         OR COALESCE(NEW.quantity_basis,'') IN ('contractor','measured');
...
BEGIN
  ...
  -- A dimension in the description is never a count. System lines only.
...
-- 3. Bind unpriced system lines to the closest catalog productivity basis.
--    This never invents a quantity and never prices a placeholder: it supplies
--    unit + hours-per-unit + material allowance so the line becomes actionable.
...
REVOKE ALL ON FUNCTION public.bind_unpriced_estimate_lines(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.bind_unpriced_estimate_lines(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.quantity_is_size_not_count(text, numeric, text) TO authenticated, service_role;

-- 4. Fold binding into the canonical repair path so every open/repair uses it.
