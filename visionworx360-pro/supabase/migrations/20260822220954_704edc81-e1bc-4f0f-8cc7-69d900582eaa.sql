CREATE OR REPLACE FUNCTION public.quantity_needs_count(_description text, _unit text, _qty numeric, _placeholder boolean)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path TO 'public'
AS $$
  SELECT COALESCE(_placeholder, false)
     AND COALESCE(_qty, 0) <= 1
     AND COALESCE(_unit,'') IN ('each')
     AND lower(COALESCE(_description,'')) ~ '\m(circuits|outlets|receptacles|shutoffs|valves|fixtures|doors|windows|cabinets|vanities|lights|lighting|switches|faucets|appliances|posts|units)\M';
$$;
...
REVOKE EXECUTE ON FUNCTION public.quantity_needs_count(text, text, numeric, boolean) FROM anon;
