  ...
  LOOP
    ...
    UPDATE public.estimate_line_items li
       SET quantity = v_quantity,
           ...
           quantity_basis = CASE
             WHEN COALESCE(si.quantity_basis, '') = 'assumed' THEN 'ballpark_allowance'
             ELSE li.quantity_basis
           END,
           ...
           labor_hours_formula = format('%s x %s hr/unit — selected ballpark snapshot',
             v_quantity, round(CASE WHEN v_quantity > 0 THEN v_labor_hours / v_quantity ELSE 0 END, 4)),
           pricing_source = 'ballpark_snapshot',
           catalog_item_key = NULL,
           catalog_mapping_source = 'ballpark_snapshot',
           priced_at = now(),
           resolution_status = 'resolved',
           unresolved_reason = NULL,
           pricing_provenance = jsonb_strip_nulls(jsonb_build_object(
             'source', 'selected_ballpark_snapshot',
             ...
             'laborHours', v_labor_hours,
             'laborCost', v_labor_cost,
             'materialCost', v_material_total,
             'otherCost', v_other_total,
             'total', COALESCE(NULLIF(v_task->>'total', '')::numeric,
                               v_labor_cost + v_material_total + v_other_total),
             'appliedAt', now()
           ))
