> Moved into this repository in Lovable Exit Phase 1. Paths are now: suite `src/tests/integration/pricingInheritance/`, harness `scripts/integration-harness/`, repair migration `supabase/migrations/20260923140000_pricing_inheritance_repair.sql`, results `docs/recovery/results/`. Run with `bun run test:integration`; the host-repo `@ts-nocheck` guard lines are gone.
